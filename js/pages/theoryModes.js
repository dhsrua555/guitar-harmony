  ...
  const page = {
    ...
    render(el, params) {
      ...
      if (params.id || (qy.mode && !qy.tab)) {
        ...
        el.appendChild(h('div', { class: 'toolbar' }, h('label', null, '루트', A.rootSelect(root, v => GH.router.go('/theory/modes/' + encodeURIComponent(cur.id), { root: v })) ), h('a', { class: 'btn small', href: '#/theory/modes' }, '모드 비교표 →')));
        el.appendChild(modeInfo(cur, root));
        return;
      }
      el.appendChild(h('h1', null, '모드 (선법)'));
      el.appendChild(h('p', { class: 'muted' }, '특정 음을 중심음(토닉)으로 삼아 고유한 도수 구조와 중심감을 만드는 스케일입니다. 밝기 순서, 평행·상대 비교로 차이를 먼저 보고 각 모드의 상세 페이지로 들어가세요.'));
      el.appendChild(callout(h('b', null, '두 가지 관점. '), h('b', null, '상대 접근'), ': C 메이저와 D 도리안처럼 같은 음 집합을 다른 루트로 보는 방식. ', h('b', null, '평행 접근'), ': C 아이오니안과 C 도리안처럼 같은 루트에서 다른 도수를 비교하는 방식입니다. 실제 연주에서는 특징음과 중심 코드를 함께 듣는 것이 중요합니다.'));
      ...
      if (state.tab === 'diatonic') {
        ...
        body.appendChild(h('div', { class: 'toolbar' }, h('label', null, '루트', A.rootSelect(root, v => GH.router.go('/theory/modes', { root: v, tab: 'diatonic' })))));
      } else if (state.tab === 'parallel') {
        const order = GH.data.diatonicModeOrder;
        const ion = GH.scales.get('ionian').intervals.map(N.ivSemi);
        body.appendChild(h('div', { class: 'toolbar' }, h('label', null, '루트', A.rootSelect(root, v => GH.router.go('/theory/modes', { root: v, tab: 'parallel' })))));
        body.appendChild(section(N.pretty(root) + ' 기준 7모드 (평행 비교)', h('p', { class: 'muted' }, '같은 루트에서 각 모드를 비교합니다. 아이오니안과 달라지는 음을 강조했습니다.'),
        ...
        body.appendChild(section(N.pretty(root) + ' 메이저 스케일에서 파생되는 모드 (상대 비교)', h('p', { class: 'muted' }, '같은 7개 음을 다른 음에서 시작합니다. 각 모드의 토닉 코드는 그 도수의 다이어토닉 세븐 코드입니다.'),
        ...
        body.appendChild(h('div', { class: 'toolbar' }, h('label', null, '루트', A.rootSelect(root, v => GH.router.go('/theory/modes', { root: v, tab: state.tab })))));
        ...
        body.appendChild(section('모달 하모니란', h('p', null, '기능 화성(T–S–D)의 긴장과 해결 대신, 한 모드의 색깔을 유지하는 화성입니다. 특징음이 들어간 코드를 강조하고, 도미넌트의 트라이톤 해결을 피하며, 뱀프나 페달 포인트로 중심음을 붙잡습니다.'),
          table(['모드', '뱀프 (모달 진행)', '피할 것', '특징'], [['도리안', 'i7 – IV7, i7 – ii7', 'bVI (에올리안으로 들림)', 'IV7의 메이저 3도 = 특징음 6'], ['프리지안', 'i – bII, i – bII – bIII', 'V7', 'bII 코드가 특징음 b2를 담는다'], ['리디안', 'I – II, Imaj7 – II7', 'IV, V (아이오니안으로 해결)', 'II 코드의 3음 = 특징음 #4'], ['믹솔리디안', 'I – bVII, I7 – IV', 'V7 → I (메이저 해결)', 'bVII의 루트 = 특징음 b7'], ['에올리안', 'i – bVII, i – bVI – bVII', 'V7 (하모닉 마이너로 들림)', 'bVI의 루트 = 특징음 b6']])));
        body.appendChild(section('모달 인터체인지와의 차이', h('p', null, '모달 하모니는 곡이나 섹션의 중심이 특정 모드에 머무는 것이고, 모달 인터체인지는 원래 키의 중심을 유지한 채 평행 모드에서 코드를 차용하는 것입니다.'), h('a', { href: A.reharmHref('modal_interchange') }, '모달 인터체인지 →')));
