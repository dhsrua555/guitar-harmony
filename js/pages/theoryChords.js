...
(function () {
  ...
  GH.pages['/theory/chords'] = {
    ...
    render(el, params) {
      ...
      el.appendChild(tabs([{ id: 'builder', label: '코드 빌더' }, { id: 'types', label: '코드 퀄리티' }, { id: 'diatonic', label: '다이어토닉 코드' }, { id: 'tension', label: '텐션' }, { id: 'notation', label: '표기법' }, { id: 'inversion', label: '전위 · 슬래시 코드' }], state.tab, id => GH.router.go('/theory/chords', { root, q: state.q, tab: id })));
  ...
  function renderBuilder(el, root, pref) {
    ...
    el.appendChild(h('div', { class: 'toolbar' }, h('label', null, '근음', A.rootSelect(root, v => GH.router.go('/theory/chords', { root: v, q: state.q, tab: 'builder' }))), h('label', null, '코드 퀄리티', A.qualitySelect(state.q, v => GH.router.go('/theory/chords', { root, q: v, tab: 'builder' }))),
    ...
    el.appendChild(h('div', { class: 'card' },
      ...
      h('p', null, q.desc),
      h('h3', null, '구성음과 도수'),
      table(['도수', '음정', '음이름', '반음'], chord.notes.map(n => [h('span', { class: 'pill ' + n.cls }, n.iv), n.ko, N.pretty(n.name), N.ivSemi(n.iv)])),
    ...
    el.appendChild(section('코드 스케일 후보', h('div', { class: 'toc' }, fits.map(f => h('a', { href: A.scaleHref(f.scale.id, root), style: f.primary ? 'font-weight:700;color:var(--accent);border-color:var(--accent)' : '' }, f.scale.ko))), h('p', { class: 'muted' }, '강조된 항목은 일반적인 출발점입니다. 실제 선택은 조성, 코드 기능, 앞뒤 진행과 멜로디를 함께 고려하세요.')));
  }
  function renderTypes(el, root, pref) {
    const A = GH.app; const C = GH.chords;
    const by = state.group;
    el.appendChild(h('div', { class: 'toolbar' }, h('label', null, '분류', select({ options: [{ value: 'category', label: '구성별 (3화음 · sus · 7th · 텐션 …)' }, { value: 'family', label: '계열별 (메이저 · 마이너 · 도미넌트 …)' }, { value: 'level', label: '난이도별 (기초 → 고급)' }], value: by, onChange: v => { state.group = v; GH.router.go('/theory/chords', { root, q: state.q, tab: 'types', group: v }); } })), h('span', { class: 'muted' }, '코드 퀄리티 ' + C.QUALITIES.length + '개. 행을 누르면 코드 빌더로 이동합니다.')));
    ...
    el.appendChild(callout('행을 누르면 코드 빌더에서 자세히 볼 수 있습니다. 같은 구성음을 다른 근음과 심벌로 해석할 수도 있습니다: C6 = Am7, Cm6 = Am7b5. C7b9에서 근음을 생략하면 C#°7, E°7, G°7, Bb°7과 같은 음 집합이 됩니다.'));
  }
  function renderDiatonic(el, key) {
    const A = GH.app;
    const scales = [['ionian', '메이저'], ['aeolian', '자연 단음계'], ['harmonic_minor', '하모닉 마이너'], ['melodic_minor', '멜로딕 마이너'], ['dorian', '도리안'], ['mixolydian', '믹솔리디안'], ['lydian', '리디안'], ['phrygian', '프리지안']];
    el.appendChild(h('div', { class: 'toolbar' }, h('label', null, '키', A.rootSelect(key, v => GH.state.set({ key: v }))), h('label', null, '스케일', select({ options: scales.map(([id, ko]) => ({ value: id, label: ko })), value: state.diaScale, onChange: v => { state.diaScale = v; GH.router.rerender(); } })), h('label', null, select({ options: [{ value: '3', label: '3화음' }, { value: '7', label: '7화음' }], value: state.sevenths ? '7' : '3', onChange: v => { state.sevenths = v === '7'; GH.router.rerender(); } })), A.fnLegend()));
  ...
  function renderTension(el, root) {
    const A = GH.app;
    el.appendChild(section('사용 가능한 텐션', h('p', null, '사용 가능한 텐션은 코드의 기능, 코드 스케일, 멜로디와 보이싱에 따라 달라집니다. 실용적인 첫 기준으로, 구조적인 코드톤과 단9도를 이루는 음은 강박이나 지속음으로 둘 때 주의하세요. 도미넌트의 변화 텐션(b9, #9, #11, b13)은 해결 방향에 맞춰 선택합니다.'),
      table(['코드', '기본 텐션', '변화 텐션', '어보이드 노트', '메모'], [
        ['maj7', '9, 13, (#11)', '–', '11 (4음)', '11은 3음과 단9도 마찰. #11은 리디안 문맥에서 사용'],
  ...
  function renderNotation(el, root) {
    ...
    el.appendChild(section('읽는 규칙', h('ul', { class: 'plain' }, ['근음 → 트라이어드 퀄리티(생략하면 메이저, m은 마이너) → 7음 종류(7 = 단7도, maj7 = 장7도) → 텐션과 변화음 순으로 읽습니다.', '숫자 하나만 쓰인 C9, C13에는 도미넌트 7이 포함됩니다. 메이저 9 코드는 Cmaj9처럼 maj를 표시합니다.', 'add는 7음 없이 해당 음만 더한다는 뜻입니다(Cadd9 = C E G D).', 'sus는 3음을 4음 또는 2음으로 바꾼 코드입니다(C7sus4 = C F G Bb).', '변화 텐션은 C7(b9)처럼 괄호 안에 쓰거나 C7b9처럼 붙여 씁니다.'].map(t => h('li', null, t)))));
  }
  function renderInversion(el, root, pref) {
    const A = GH.app;
    const c = GH.chords.buildChord(root, 'maj');
    el.appendChild(section('전위 (Inversion)', h('p', null, '근음이 아닌 코드톤을 최저음에 두면 전위형입니다. 3음이 베이스면 1전위, 5음이면 2전위, 7음이면 3전위입니다. 기타에서는 드롭 2처럼 전위별로 서로 다른 운지 형태를 사용합니다.'),
    ...
    el.appendChild(section('슬래시 코드', h('p', null, 'C/E는 C 코드의 베이스음을 E로 지정한 표기입니다. 전위형을 나타내거나(C/E, C/G), 코드톤이 아닌 베이스음을 지정할 수 있습니다(C/D, F/G). 베이스 라인을 매끄럽게 연결할 때 자주 씁니다.'),
      h('div', { class: 'toc' }, h('a', { href: A.reharmHref('bass_line') }, '베이스 라인 리하모니제이션 →'), h('a', { href: A.reharmHref('pedal_point') }, '페달 포인트 →'))));
    el.appendChild(section('하이브리드 코드와 폴리코드', h('p', null, '베이스 위에 그 음을 포함하지 않는 3화음을 얹은 것이 하이브리드 코드 (G/C, F/G). 두 코드를 위아래로 겹친 것이 폴리코드 (D/C7 처럼 가로줄로 적기도 함). 어퍼 스트럭처 트라이어드는 도미넌트 위의 폴리코드입니다.')));
  }
})();
