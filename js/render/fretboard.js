...
(function () {
  ...
  /* opts: { tuning, from, to, lefty, pcMap{pc:{label,cls}}, notes[{s,f,label,cls,ghost,dim}], filter(s,f,pc)->'dim'|'hide'|null,
            window[lo,hi], onClick(s,f,midi), capo, labelMode('degree'|'name'), pref, showStringNames, sound(midi) 누를 때 소리 } */
  function fretboard(opts) {
    ...
    const NS = tuning.length, MID = (NS - 1) / 2;          /* 줄 수 (기타 6 · 베이스 4) */
    const nFrets = to - from;
    const FW = 62, LEFT = 46, RIGHT = 14, TOP = 14, SH = 24, BOTTOM = 26;
    const W = LEFT + nFrets * FW + RIGHT + (from === 0 ? 20 : 0), H = TOP + (NS - 1) * SH + BOTTOM;
    ...
    el.appendChild(svg('rect', { class: 'board', x: bx0, y: TOP - 8, width: bw, height: (NS - 1) * SH + 16, rx: 3 }));
    el.appendChild(S().path(S().line(bx0, TOP - 8, bx0 + bw, TOP - 8, { passes: 1, bow: 0.6 }) + S().line(bx0, TOP + (NS - 1) * SH + 8, bx0 + bw, TOP + (NS - 1) * SH + 8, { passes: 1, bow: 0.6 }), 'board-edge'));
    /* 인레이 */
    [3, 5, 7, 9, 12, 15, 17, 19, 21, 24].forEach(f => {
      if (f <= from || f > to) return;
      const x = mx(noteX(f)); const cy = TOP + MID * SH;
    ...
    for (let f = from; f <= to; f++) {
      const x = mx(fretX(f));
      if (f === 0) el.appendChild(S().path(S().line(x, TOP - 6, x, TOP + (NS - 1) * SH + 6, { passes: 3, jitter: 1.4 }), 'nut'));
      else el.appendChild(S().path(S().line(x, TOP - 8, x, TOP + (NS - 1) * SH + 8, { passes: 2, bow: 0.7 }), 'fret'));
    ...
    if (capo > from && capo <= to) {
      ...
      el.appendChild(S().path(S().blob(cxp, TOP + MID * SH, 4, MID * SH + 10, { points: 10, wobble: 0.04 }), 'capo', { fill: 'var(--accent)', opacity: .85 }));
      el.appendChild(S().path(S().ellipse(cxp, TOP + MID * SH, 4.6, MID * SH + 10.5, { points: 12, wobble: 0.03 }), 'sk-ink'));
    }
    /* 줄 */
    for (let s = 1; s <= NS; s++) {
    ...
    if (opts.window) {
      ...
      if (b >= a) {
        ...
        el.appendChild(svg('rect', { class: 'window-fill', x: wx, y: TOP - 11, width: ww, height: (NS - 1) * SH + 22, rx: 6 }));
        el.appendChild(S().path(S().rect(wx, TOP - 11, ww, (NS - 1) * SH + 22, { passes: 1, bow: 0.8 }), 'window'));
      }
    }
    /* 클릭 영역: 어디를 눌러도 그 음이 울린다. onClick 이 있으면 (코드 파인더) 함께 호출 */
    const playable = opts.playable !== false;
    const flash = (s, f) => { const g = noteEls[s + ':' + f]; if (g) { g.classList.remove('hit'); void g.getBBox(); g.classList.add('hit'); setTimeout(() => g.classList.remove('hit'), 420); } else { const midi = tuning[NS - s] + f; const tmp = dot(s, f, N.noteName(midi % 12, opts.pref || 'sharp'), 'iv-1', { ghost: true }); tmp.classList.add('hit', 'temp'); setTimeout(() => { if (tmp.parentNode) tmp.parentNode.removeChild(tmp); delete noteEls[s + ':' + f]; }, 480); } };
    if (opts.onClick || playable) {
      for (let s = 1; s <= NS; s++) for (let f = from; f <= to; f++) {
        if (f < capo) continue;
        const midi = tuning[NS - s] + f;
        const activate = e => {
          if (e.type === 'keydown' && e.key !== 'Enter' && e.key !== ' ') return;
          if (e.type === 'keydown') e.preventDefault();
          if (playable && GH.audio) { if (opts.sound) opts.sound(midi); else GH.audio.pluck(midi, GH.audio.now(), 1.4, { gain: 0.9 }); flash(s, f); }
    ...
    if (opts.pcMap) {
      for (let s = 1; s <= NS; s++) for (let f = from; f <= to; f++) {
        if (f < capo) continue;
        const midi = tuning[NS - s] + f; const pc = mod(midi, 12);
    ...
    (opts.notes || []).forEach(n => {
      ...
      const midi = tuning[NS - n.s] + n.f; const pc = mod(midi, 12);
    ...
    return {
      ...
      highlightMany(pairs) {
        ...
        (pairs || []).forEach(([s, f]) => {
          let g = noteEls[s + ':' + f];
          if (!g) { const midi = tuning[NS - s] + f; g = dot(s, f, N.noteName(midi % 12, opts.pref || 'sharp'), 'iv-1', {}); }
