...
(function () {
  ...
  /* opts: {from: midi, to: midi, on: {pc: {label, cls}} 또는 onMidi: {midi:{label,cls}}, pref, onClick(midi), preset(누를 때 음색)}
     돌려주는 svg 에 highlightMany([midi…]) 가 붙어 있다 (연주 중 누르는 건반 표시) */
  function piano(opts) {
    opts = opts || {};
    ...
    let x = 1; const xs = {}; const keyEls = {};
    for (let m = from; m <= to; m++) {
      if (BLACK.has(mod(m, 12))) continue;
      const i = info(m);
      const activate = e => { if (e.type === 'keydown' && e.key !== 'Enter' && e.key !== ' ') return; if (e.type === 'keydown') e.preventDefault(); if (playable && GH.audio) { GH.audio.pluck(m, GH.audio.now(), 1.4, { gain: 0.9, preset: opts.preset }); flash(g); } if (opts.onClick) opts.onClick(m); };
      ...
      el.appendChild(g); xs[m] = x; keyEls[m] = g; x += WW;
    }
    for (let m = from; m <= to; m++) {
      if (!BLACK.has(mod(m, 12))) continue;
      ...
      el.appendChild(g); keyEls[m] = g;
    }
    S().grain(el, W, H);
    let cur = [];
    el.highlightMany = ms => { cur.forEach(g => g.classList.remove('current')); cur = (ms || []).map(m => keyEls[m]).filter(Boolean); cur.forEach(g => g.classList.add('current')); };
    return el;
  }
